...
2、英文字母：[a-zA-Z]
3、数字：[0-9]
4、匹配中文，英文，数字及下划线：^[\u4e00-\u9fa5_a-zA-Z0-9]+$
...
7、由数字、字母或者下划线组成的字符串：
8、2~4个汉字：
9、最长不得超过7个汉字，或14个字节(数字、字母、下划线)正则表达式：
10、匹配双字节字符(包括汉字在内)：
...
13、匹配首尾空白字符的正则表达式：
   评注：可以用来删除行首行尾的空白字符串(包括空格、制表符、换页符)，非常有用的表达式
14、匹配Email地址的正则表达式：
   评注：表单验证时很实用
15、手机号：
16、身份证号：
17、匹配网址URL的正则表达式：
   评注：
18、匹配账号是否合法(字母开头，允许5-16字节，允许字字母数字下划线)：
```

<img src="./images/022.jpg" alt="image" style="zoom:80%;" />

<img src="./images/023.jpg" alt="images" style="zoom:80%;" />
